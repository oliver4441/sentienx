...
const STRATEGY_PRESETS = [
  {
    value: "conservative",
    label: "Conservative",
    desc: "High confluence threshold, low Kelly fraction, small stakes",
    confluence: 0.75,
    kelly: 0.2,
    maxStakePct: 0.02,
  },
  {
    value: "moderate",
    label: "Moderate",
    desc: "Balanced confluence, medium Kelly, moderate stakes",
    confluence: 0.6,
    kelly: 0.35,
    maxStakePct: 0.05,
  },
  {
    value: "aggressive",
    label: "Aggressive",
    desc: "Lower confluence, full Kelly, larger stakes",
    confluence: 0.45,
    kelly: 0.5,
    maxStakePct: 0.08,
  },
...
function BotCard({
  ...
  const [step, setStep] = useState<"idle" | "config" | "confirm">("idle");
  ...
  const isConfiguring = step === "config" || step === "confirm";
  ...
  const applyPreset = (presetValue: string) => {
    ...
    const preset = STRATEGY_PRESETS.find((p) => p.value === presetValue);
    if (preset) {
      setMinConfluenceScore(preset.confluence);
      setKellyFraction(preset.kelly);
    }
  };

  const handleStart = () => {
    const preset = STRATEGY_PRESETS.find((p) => p.value === selectedPreset);
    onStart({
      ...
      minConfluenceScore,
      kellyFraction,
      strategy: selectedPreset as "conservative" | "moderate" | "aggressive",
      demo: demoMode,
    });
    setStep("idle");
  };
  ...
  return (
    <div className={`stat-card ${isRunning ? "border-green-500/30" : ""}`}>
      ...
      {/* ─── Configuration Panel ─── */}
      {isConfiguring && isStopped && (
        <div className="space-y-3 mb-3 p-3 rounded-lg bg-[#070709]/50 border border-white/[0.04]">

          {/* Step 1: Symbol & Mode */}
          {step === "config" && (
            <>
              <div className="flex items-center justify-between mb-1">
                <p className="text-xs font-semibold text-[#818cf8]">Step 1: Market & Mode</p>
                <span className="text-[10px] text-[#71717a]">1/3</span>
              </div>
                ...
                <button
                  ...
                  className={`flex-1 py-2.5 rounded-lg text-xs font-medium transition-all min-h-[44px] ${
                    demoMode
                      ...
                      : "bg-[#070709] border border-white/[0.08] text-[#71717a]"
                  }`}
                >
                  Demo Mode
                </button>
              ...
              {!demoMode && (
                <div className="mb-2 p-2 rounded-lg bg-yellow-500/10 border border-yellow-500/20">
                  <p className="text-[10px] text-yellow-400">
                    Live mode will execute real trades with your Deriv account. Make sure you understand the risks.
                  </p>
                </div>
              )}
              ...
              <button
                ...
                className="w-full py-2.5 rounded-lg text-xs font-medium bg-[#6366f1] hover:bg-[#4f46e5] text-white transition-colors min-h-[44px]"
              >
                Next: Strategy & Risk
              </button>
            </>
          ...
          {step === "confirm" && (
            <>
              <div className="flex items-center justify-between mb-1">
                <p className="text-xs font-semibold text-[#818cf8]">Step 2: Strategy & Risk</p>
                <span className="text-[10px] text-[#71717a]">2/3</span>
              </div>
                    ...
                    <button
                      ...
                      className={`w-full text-left p-2.5 rounded-lg border transition-all ${
                        selectedPreset === preset.value
                          ...
                          : "bg-[#070709] border-white/[0.06] hover:border-white/[0.12]"
                      }`}
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-medium text-[#f4f4f5]">{preset.label}</span>
                        {selectedPreset === preset.value && (
                          <span className="text-[10px] text-[#818cf8]">Selected</span>
                        )}
                      </div>
                      <p className="text-[10px] text-[#71717a] mt-0.5">{preset.desc}</p>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="border-t border-white/[0.06] pt-2">
                <p className="text-[10px] sm:text-xs text-[#818cf8] font-semibold mb-2">Risk Limits</p>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  <div>
                    <label className="text-[10px] text-[#71717a] block mb-1">Take Profit ($)</label>
                    <input type="number" value={takeProfit} onChange={(e) => setTakeProfit(Number(e.target.value))} min={1}
                      className="w-full bg-[#070709] border border-white/[0.08] rounded-lg px-2.5 py-2.5 text-sm min-h-[44px]" />
                  </div>
                  <div>
                    <label className="text-[10px] text-[#71717a] block mb-1">Stop Loss ($)</label>
                    <input type="number" value={stopLoss} onChange={(e) => setStopLoss(Number(e.target.value))} min={1}
                      className="w-full bg-[#070709] border border-white/[0.08] rounded-lg px-2.5 py-2.5 text-sm min-h-[44px]" />
                  </div>
                  <div>
                    <label className="text-[10px] text-[#71717a] block mb-1">Max Consec. Losses</label>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 py-2.5 rounded-lg text-xs font-medium bg-[#070709] border border-white/[0.08] hover:border-white/[0.15] transition-colors min-h-[44px]"
                >
                  Back
                </button>
                <button
                  onClick={handleStart}
                  className="flex-1 py-2.5 rounded-lg text-xs font-medium bg-[#00e676] hover:bg-[#00c853] text-black font-semibold transition-colors min-h-[44px]"
                >
                  {demoMode ? "Start Demo" : "Start Live Bot"}
                </button>
              </div>
            </>
          )}
